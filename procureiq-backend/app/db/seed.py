...
Always uses the CURRENT calendar year so data never looks stale.
"""
...
async def seed() -> None:
    # Always use current year so data is never stale
    ...
    async with async_session_factory() as db:
        ...
        # ── Spend Transactions (12 months of current year) ───────────────────
        start_date = date(current_year, 1, 1)
        # End at yesterday so we never go into the future
        end_date   = min(date(current_year, 12, 31), date.today() - timedelta(days=1))
        if end_date <= start_date:
            # Edge case: running on Jan 1 — use previous year
        ...
        for i, (sid, (name, *_, spend)) in enumerate(zip(supplier_ids, SUPPLIERS_DATA)):
            ...
            for _ in range(n_tx):
                ...
                db.add(SpendTransaction(
                    id=str(uuid.uuid4()),
                    supplier_id=sid,
                    po_number=f"PO-{year_str}-{random.randint(10000,99999)}",
        ...
        # ── Contracts (use current year dates) ────────────────────────────────
        today = date.today()
        ...
        for i, (sid, (name, *_, spend)) in enumerate(zip(supplier_ids[:30], SUPPLIERS_DATA[:30])):
            ...
            if cs == "expired":
                s = date(prev_year, 1, 1)
                e = date(prev_year, 12, 31)
            elif cs == "expiring_soon":
                # expiring within next 30–89 days
                days_ahead = random.randint(15, 89)
                e = today + timedelta(days=days_ahead)
                s = e - timedelta(days=365)
            elif cs == "draft":
                s = today + timedelta(days=30)
                e = today + timedelta(days=365 + 30)
            else:  # active
                s = date(current_year, 1, 1)
                e = today + timedelta(days=random.randint(90, 730))
            db.add(Contract(
                id=str(uuid.uuid4()),
                supplier_id=sid,
        ...
        await db.commit()
        print(f"[OK] Seeded {len(users)} users, {len(supplier_ids)} suppliers, contracts, and spend transactions for {year_str}.")
